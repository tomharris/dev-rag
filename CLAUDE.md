...
Python 3.12+. Uses `hatchling` build backend. Key deps: `qdrant-client`, `fastembed` (BM25 sparse encoder), `fastmcp`, `tree-sitter` + `tree-sitter-language-pack`, `sentence-transformers`, `typer`, `httpx`, `gitpython`, `browser-cookie3` (reads the browser `d` cookie for `devrag auth slack`). Dev extra: `pytest`, `pytest-asyncio`, `respx`.
