...
- `query_router.py` - Regex-based intent classification routes queries to relevant collections. On the auto-route (`scope="all"`) path, a doc-intent match (e.g. "architecture", "describe the") routes to `documents` **plus** `code_chunks` so broad questions don't silently exclude code; an explicit `scope="docs"` still narrows to docs only.
...
Python 3.12+. Uses `hatchling` build backend. Key deps: `qdrant-client`, `fastembed` (BM25 sparse encoder), `fastmcp`, `tree-sitter` + `tree-sitter-language-pack`, `sentence-transformers`, `typer`, `httpx`, `gitpython`. Optional extras: `slack-auth` (`browser-cookie3`, for `devrag auth slack`). Dev: `pytest`, `pytest-asyncio`, `respx`.
